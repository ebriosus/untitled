...
OvrPro::OvrPro(){
    cameraMode = OVR::OV_CAMVR_VGA; //960x950 @60fps
    
    //Create Ovrvision object
       //ovr_Ovrvision.reset(new OVR::OvrvisionPro); // when use smart pointer
	OVR::OvrvisionPro ovr_Ovrvision; // when use regular pointer
    ofSleepMillis(1000);
...
void OvrPro::init(){
    ...
    try{
        if (ovr_Ovrvision.Open(0, OVR::OV_CAMVR_VGA, 0, NULL)){
	...
	ovr_Ovrvision.SetCameraGain(30);
	ovr_Ovrvision.SetCameraWhiteBalanceAuto(true);
...
void OvrPro::greyscale(unsigned char *left, unsigned char *right) {
	ovr_Ovrvision.Grayscale(left, right);
}
