...
//add tracking option
std::vector<bbox_t> ofxYolo2::tracking(std::vector<bbox_t> cur_bbox_vec) {
	
	cur_bbox_vec = detector->tracking(cur_bbox_vec);
...
void ofxYolo2::setup() {
	...
	string config = ofToDataPath("yolo.cfg");
	string weights = ofToDataPath("yolo.weights");
	string names = ofToDataPath("coco.names");
